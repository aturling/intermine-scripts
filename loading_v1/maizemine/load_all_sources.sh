...
# load_all_sources.sh
#
# Load all from project.xml for maizemine v1.3
########################################################
...
if [ $ec -eq 0 ]; then
    ...
    # restart before this one - needs lots of connections apparently
    ...
    echo >> $outfile
    echo "$(timestamp) Loading (minus publications) completed" >> $outfile
